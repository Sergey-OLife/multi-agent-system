# Optional Workflow Layer — source_intake_auditor
...
`source_intake_auditor` включается как optional workflow layer.

Он не является hard guardrail, не является route-required режимом и не является `workflow_conductor_agent`.

Его задача:

> Источник не работает, пока не понятно, что это, где лежит, зачем нужен и чего из него нельзя брать.

Слой проверяет статус источника, отделяет карточку от материала, targeted reading от full audit, дубль от canonical source и готовит orchestration fields для будущего `workflow_conductor_agent`.
...
- автоматический Source Intake Audit всех источников без отдельного плана.
...
- `knowledge/05_agent_memory/activation_proposals/source_intake_controlled_activation.md`
