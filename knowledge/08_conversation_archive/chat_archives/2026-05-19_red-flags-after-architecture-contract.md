...
Implemented elsewhere: partial / PR #129, PR #131, PR #136, project-state v2.25
...
В разговоре стало ясно: часть красных флагов уже закрыта или снижена через baseline CI, repository architecture contract и command protocol, но несколько хвостов остаются рабочими рисками.
...
- Уже отражено:
  - Где: PR #136 — `Add archive start GitHub write command`
  - Что НЕ нужно дублировать: `#архив_старт`, GitHub-only archive destination, запрет memory/handoff/project-state, quarantine rule для массовых capture PR.

- Уже отражено:
  - Где: `knowledge/00_manifest/project-state.json` v2.25
  - Что НЕ нужно дублировать: state sync после PR #131 и PR #136 уже выполнен через PR #137.
...
- Идея:
  ...
  - Куда может перейти: roadmap / repository hygiene issue / README PR / branch protection checklist

- Идея:
  - Суть: отсутствие README частично компенсировано architecture contract, но не закрыто полностью.
  - Почему может быть важна: architecture contract — внутренняя несущая стена; README — входная дверь для future-you, Codex, ChatGPT и внешнего ревьюера.
...
- Хвост:
  - Что осталось не сделано: проверить, как baseline CI отработал на PR #131 или на следующем реальном PR.
  - Почему не сделано: CI внедрён, но отдельный post-merge разбор ещё не закреплён.
  - Что нужно для продолжения: посмотреть checks/workflow result, если tool surface даёт доступ.
...
- Хвост:
  ...
  - Что нужно для продолжения: определить минимальный первый check: presence/consistency check для обязательных paths, state version, archive index, working protocol references.
...
- Сбой:
  - Что произошло: PR #133 устарел относительно `main` после PR #136/#137 и стал unmergeable.
  - Почему это важно: даже archive PR нельзя мержить из старой ветки, если index/state изменились.
  - Как избегать: создавать replacement PR от свежего `main`, а старый закрывать как superseded.
  - Нужно ли внести в protocol: no — это покрывается idempotency/race-condition rules.
...
- Противоречие:
  ...
  - Что спросить у Сергея позже: включаем ли branch protection после проверки CI?
...
- Противоречие:
  ...
  - Что спросить у Сергея позже: делаем ли README следующим?
...
- Replacement PR for this archive entry is not permission to merge future README / branch protection work.

## 10. Рекомендованный следующий шаг

После merge archive entry выбрать один следующий шаг:
