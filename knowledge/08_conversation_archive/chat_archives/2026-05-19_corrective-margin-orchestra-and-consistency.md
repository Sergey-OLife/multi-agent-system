# Conversation Archive Entry — corrective-margin-orchestra-and-consistency

Дата: 2026-05-19
Источник: current_chat_summary
Статус: draft_archive_entry
Срок пересмотра: 2026-06-02
Tags: [agent_shipyard, critic_margin_agent, margin_orchestra, archive_protocol, knowledge_consistency, open_loop, architecture, failure_pattern, corrective]
Implemented elsewhere: partial / PR #138, PR #140, PR #142, project-state v2.25 pending state sync

## 0. Coverage check

- Coverage scope: corrective
- Previous checkpoint: `knowledge/08_conversation_archive/chat_archives/2026-05-19_red-flags-after-architecture-contract.md` from PR #138.
- Previous checkpoint coverage scope: thematic.
- Previous archive/state coverage status: complete for red-flags / repository-risk discussion only; not proven complete for the whole chat.
- Full-chat marker present: no.
- Gap found: yes.
- What this entry covers: corrective capture of the known unarchived semantic tail that appeared after and around the red-flags discussion, including archive stabilization, `conversation_archive_librarian`, parallel archive intake, checkpoint delta sync, `critic_margin_agent` / `margin_orchestra`, cumulative-capture failure, explicit `coverage_scope` fix, and `Баги будущего` / knowledge consistency risk.
- What remains outside this entry: any earlier current-chat material before the red-flags thematic entry that was not independently verified as covered; implementation PRs for agent proposals, state sync after PR #138/#140/#142, README, branch protection, protocol/knowledge consistency audit, and future Go validator.

## 1. Почему этот архив создан

После merge red-flags archive entry появились новые смысловые линии. Первая попытка `#архив_старт` создала PR #139 только по `critic_margin_agent / margin_orchestra`, то есть сработала как last-topic-only. PR #140 исправил это, закрепив cumulative capture.

Затем PR #141 попытался сделать cumulative replacement, но Сергей указал на более глубокий дефект: нельзя считать `2026-05-19_red-flags-after-architecture-contract.md` границей полного архива, потому что в ней не было маркера `coverage_scope: full_chat`. Она была корректной тематической записью, но не full-chat checkpoint.

PR #142 исправил protocol bug: no full-chat marker = thematic coverage by default. Этот entry создан как corrective archive, а не как pretend-full archive.

## 2. Что уже отражено в архитектуре

- Уже отражено:
  - Где: PR #138 — `Archive red flags after architecture contract`
  - Что НЕ нужно дублировать: красные флаги repository risks уже сохранены как thematic archive entry.

- Уже отражено:
  - Где: PR #140 — `Require cumulative archive start capture`
  - Что НЕ нужно дублировать: правило `#архив_старт = cumulative capture, not last-topic-only` уже внедрено.

- Уже отражено:
  - Где: PR #142 — `Require explicit archive coverage scope`
  - Что НЕ нужно дублировать: archive entry нельзя считать full-chat checkpoint без explicit `coverage_scope: full_chat`; thematic entry is default when no marker exists.
...
- Частично отражено:
  ...
  - Что ещё нельзя считать implemented: state sync after PR #138, PR #140 and PR #142 is not done yet; project-state still points to PR #136 as lastMergedPr.

- Не implemented:
  - Где: PR #141 open / HOLD
  - Что ещё нельзя считать implemented: PR #141 lacks explicit `coverage_scope` and should be closed as superseded after this corrective replacement exists.

## 3. Ключевые идеи, которые иначе потеряются

- Идея:
  - Суть: archive entries need coverage semantics, not just dates and topics.
  - Почему может быть важна: without `coverage_scope`, a thematic entry can be mistaken for a full-chat checkpoint.
  - Статус: implemented_elsewhere / PR #142 protocol fix
  - Куда может перейти: archive audit validation later

- Идея:
  - Суть: `Conversation Archive Stabilization Wave` should become a roadmap block.
  - Почему может быть важна: after mass-running archive commands across 13 chats, the archive flow needs structured quarantine, coverage scope and consolidation, not just more entries.
...
- Идея:
  ...
  - Почему может быть важна: this role would monitor archive index, duplicates, quarantine PRs, stale entries, `implemented_elsewhere`, 14-day review and prevent archive from becoming a dump.
...
- Хвост:
  - Что осталось не сделано: close PR #141 as superseded.
  - Почему не сделано: it remains open / HOLD and lacks explicit `coverage_scope`.
  - Что нужно для продолжения: after this corrective PR is opened, close PR #141 without merge.

- Хвост:
  - Что осталось не сделано: state sync after PR #138, PR #140, PR #142 and this corrective archive PR.
  - Почему не сделано: archive command is not state sync.
  - Что нужно для продолжения: separate state sync PR after merge.
...
- Хвост:
  ...
  - Почему не сделано: `Баги будущего` was assessed; no PR yet.
...
- Наблюдение:
  ...
  - Как учитывать: when he says `#архив_старт`, check prior coverage scope and include all unresolved semantic seeds since the last verified full-chat point; if none exists, state the gap.
...
- Наблюдение:
  - Поведение / предпочтение: Сергей requires explicit evidence for archive coverage claims.
  - Как учитывать: do not infer “whole chat covered” from an archive entry unless it says so.
  - Риск неправильного применения: trusting archive index titles or dates as proof of coverage.
  - Может перейти в: archive governance / audit rule
...
- Сбой:
  ...
  - Почему это важно: it omitted earlier unarchived ideas after the red-flags archive point.
...
- Сбой:
  - Что произошло: PR #141 treated red-flags archive entry as a previous checkpoint without enough coverage-scope discipline.
  - Почему это важно: red-flags entry was thematic, not proof of full-chat coverage.
  - Как избегать: PR #142 added `coverage_scope` and full-chat marker rules.
  - Нужно ли внести в protocol: already done by PR #142.

- Сбой:
  - Что произошло: there is still stale state after PR #138/#140/#142 because project-state has not been synced.
...
- Формула: `Без full_chat marker архивная запись — тема, а не граница мира.`
  - Где применить: archive governance / capture prompt.
  - Ограничение: metaphor only; must be backed by coverage_scope field.
...
- PR #141 remains open / HOLD and is not implemented.
- PR #142 fixed archive protocol, but project-state still needs sync after PR #138/#140/#142.
- This entry does not claim full coverage of earlier current-chat history before the red-flags thematic entry.

## 10. Рекомендованный следующий шаг

After this corrective archive PR is opened:

1. Close PR #141 as superseded.
2. Merge this corrective archive PR after explicit `++`.
3. Then perform state sync after PR #138, PR #140, PR #142 and this corrective archive PR.
...
- Claims that this entry is full-chat coverage.
