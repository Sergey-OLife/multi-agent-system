...
|---:|---|---|---|---|
| 1 | GitHub as source of truth is not atomicity-safe. | `already_fixed` / `keep_watch` | Correct risk. Current mitigation exists through state-sync discipline, restart from GitHub main, and warning-only drift audit. It is not transaction-safe and should not be described as such. | Keep manual state-sync discipline and warning-only drift audit; do not build database state now. |
| 2 | Manual discipline is not automation, but workflow relies on it. | `already_fixed` / `keep_watch` | Correct. Project now repeatedly states manual disciplines are not validators, hard guardrails, routes, or approval bypasses. | Continue enforcing this in wording and PR review. |
...
| 6 | State sync after merge is a Damocles sword. | `already_fixed` / `keep_watch` | Real risk. Already partially addressed by drift detector proposal, local warning-only script, tests, and recent state-sync practice. | Keep warning-only. Do not make it blocking without separate decision. |
...
| 8 | Lifecycle words are semantic traps. | `already_fixed` / `partly_future` | Partly stale: lifecycle contracts v1 now exists in `go-core/lifecycle/`. But it remains pure vocabulary, not policy layer. | Do not implement policy layer without separate decision. |
| 9 | "Do not confuse" warnings reveal likely failure points. | `already_fixed` / `keep_watch` | Correct. The project now uses repeated boundary language because these confusions are high-risk. | Keep boundary language precise; avoid making warnings sound like automation. |
...
| 12 | `assistant_codex_worklog` is a second brain and drift-prone. | `already_fixed` / `keep_watch` | Correct by design. Worklog is the recovery layer, but GitHub main remains source of truth. | Continue restart protocol and state sync. |
...
| 15 | Workflows are the only hard guardrail. | `already_fixed` / `keep_watch` | Correct. Minimal branch protection requires PRs and required checks. Manual disciplines remain non-hard. | Keep this distinction explicit. |
...
| 18 | Many branches and no releases mean no frozen version. | `true_but_future` / `partly_current` | Branch hygiene is a known operational matter. Releases/tags are not required while the repo is internal project OS. | Branch cleanup only through safe UI/tool. Release discipline only if external reuse becomes real. |
| 19 | Scripts are more active than Go-core. | `already_fixed` / `reframe` | Recent scripts/core audit answered the direct risk: current scripts remain edge automation / CI helpers. | No implementation from this point. |
...
| 21 | Main should always be synchronized with state. | `already_fixed` / `keep_watch` | The project cannot guarantee atomicity, but it has explicit state-sync and restart discipline. | Keep using state-sync PRs after state-sensitive merges. |
| 22 | Manual discipline is expected but not enforced. | `already_fixed` / `keep_watch` | True and intentional. Manual discipline is not enforcement. | Do not oversell it. Reviewer/bot comments protocol now strengthens manual checking. |
...
| 25 | CI protects main; if CI fails, main is exposed. | `already_fixed` / `keep_watch` | Minimal branch protection now requires PR and checks. This is repository protection, not production security. | Keep checks green and exact contexts documented. |
...
| 27 | Delete README and replace it with index links. | `not_appropriate_current` / `reframe` | Do not delete README. The useful signal is to keep README as entrance map and move detail outward. | Consider future README-boundary cleanup, not deletion. |
...
| 31 | Convert lifecycle words into types. | `already_fixed` / `partly_future` | Lifecycle contracts v1 already exists as pure Go vocabulary with tests. Full policy layer remains future-only. | Do not reopen as broad implementation. |
...
| 33 | Entity-to-stage matrix is needed. | `already_fixed` / `partly_future` | Lifecycle v1 currently uses allowed/forbidden entity-stage pairs for selected entity types. Broader PR/entity policy matrix is future work. | Do not expand without separate approval. |
